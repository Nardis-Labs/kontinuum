...
>NOTE: This project is in early stage development. Follow us to stay up to date on its development. 
...
```sh
make docker-build docker-push IMG=<some-registry>/kontinuum:tag
```
...
```sh
make deploy IMG=<some-registry>/kontinuum:tag
```
...
```sh
make build-installer IMG=<some-registry>/kontinuum:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/kontinuum/<tag or branch>/dist/install.yaml
```
